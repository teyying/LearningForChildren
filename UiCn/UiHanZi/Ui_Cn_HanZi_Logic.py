...
class UiCn_HanZi_Logic(QWidget, Ui_Cn_HanZi):
    def __init__(self, parent):
        ...
        self.dataHanZi = load(open('UiCn/UiHanZi/hanZi.json', 'r', encoding='utf-8'))
    ...
    def slot_btns_hanzi(self):
        ...
        movie = QMovie(f"UiCn/UiHanZi/image/hanZi/{text}.gif")  # 显示笔画演示GIF
                ...
                if text == '添加':  # 没有此汉字时添加
                    ...
                    for i in getText:
                        if i in self.dataHanZi.keys():  # 有此汉字时删除
                            ...
                            self.failed += i
                    # 删除完数据，重新写入json文件
                    dump(self.dataHanZi, open('UiCn/UiHanZi/hanZi.json', 'w', encoding='utf-8'), indent=4,
                         ensure_ascii=False)
    ...
    def crawlGif(self, _str):
        ...
        listGif = listdir('UiCn/UiHanZi/image/hanZi')
        tempData = {}
        try:
            for i in _str:
                fileName = f'{i}.gif'
                if fileName not in listGif or i not in self.dataHanZi.keys():  # 如果gif图片和字符串数据都没有
                    ...
                    if fileName not in listGif:
                        ...
                        open(f'UiCn/UiHanZi/image/hanZi/{fileName}', 'wb').write(gif_wbdate)  # 把下载的gif放进文件夹内
            ...
            self.mainEventLog([f"{getcwd()}\\UiCn\\UiHanZi\\UiCn_HanZi_Logic.py >>>UiCn_HanZi_Logic.crawlGif", e])
