...
class UiGames_Puzzle_Logic(QWidget, Ui_Puzzle):
    ...
    def loadPix(self, savePos=False):
        self.btnFinish.hide()
        image = choice(listdir('UiGames/UiPuzzle/image/bg'))  # 随机选择指定目录中的一张拼图
        bgImage = choice(listdir('UiGames/UiPuzzle/image/bg2'))  # 随机选择指定目录中的一张背景图(拼图区)

        pix = QPixmap(f'UiGames/UiPuzzle/image/bg/{image}').scaled(400, 400)  # 把图片缩放为400*400,如果有水印,还需要PS处理
        self.btnAnimat.setStyleSheet(f"border-image: url('UiGames/UiPuzzle/image/bg/{image}');")
        self.frameBg.setStyleSheet(f"border-image: url('UiGames/UiPuzzle/image/bg2/{bgImage}');")
