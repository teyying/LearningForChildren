...
class UiGames_CardMatching_Logic(QWidget, Ui_CardMatching):
    def __init__(self, parent):
        ...
        self.btnFinish.setStyleSheet(
            "border-images: url('game/cardMatching/images/3.png');border-radius:5px;font-size:100px;color:cyan;")
    ...
    def updateCard(self, num):
        ...
        listImgNames = listdir("game/cardMatching/images/奥特曼")
...
class MyBtn(QPushButton):
    ...
    def __init__(self, parent, pixPathBg, pixPathFace):
        ...
        self.pixBg = QPixmap(f"game/cardMatching/images/{pixPathBg}").scaled(self.size())
        # 创建一个翻转180度的QPixmap(卡片正面)，如果不提前翻转，lab_paintEvent绘制事件的翻转动作结束后，图片会左右变反
        self.pixFace = QPixmap(f"game/cardMatching/images/奥特曼/{pixPathFace}").scaled(self.size()).transformed(
